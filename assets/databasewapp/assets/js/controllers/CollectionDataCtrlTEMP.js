'use strict';
/*addition*/
app.controller("CollectionDataCtrl", ["$scope", "$http", "$timeout", "$stateParams", "SweetAlert", "info", "$state", 'FileUploader','ngNotify', function($scope, $http, $timeout, $stateParams, SweetAlert, info, $state, FileUploader, $window,ngNotify) {
     $scope.currentStep = 1;
...
autoclose: true
 ...
 $('#txtcollection_end_date').datepicker('setDate', enddate);
        ...
        /*
           ...
           $('#txtcollection_end_date').datepicker('setDate', enddate).on('changeDate', function(e) {
...
/*
    var viewValue = 7;
     ...
     $scope.collectors = function(viewValue) {
        return $http.get('./assets/views/action/getcollectorsarr.php?query=' + "7").then(function(res) {
            return res.data;
        });
    }
    /*
         $http({
        method: 'GET',
        url: 'assets/views/action/getcollectorsarr.php'
    ...
    if (result.length===0){
        ...
        var getIsLiked = function(i){
         ...
         $http.get(url,{responseType: "blob"}).success(function(data, status, headers, config){
            ...
            var file = new File([data], iname,{type:mimetype,beconfuse:beconfuse});
...
/*
        ...
        var pmeters = "tcoll_id=" + collectionid +
            ...
            "&tcoll_specific_locality=" + document.getElementById("txtspecific_locality").value + 
            "&tcoll_locality=" + encodeURI(document.getElementById("txtlocality").value) + 
            "&tcoll_thabitat=" + encodeURI(document.getElementById("txthabitat").value) + 
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        SweetAlert.swal({
                            ...
                            uploaderImages.onBeforeUploadItem = function (item) {
                            item.formData = [{ coll_id: $scope.collectionid, secondParam: 'value' }];
                            };   
                            ...
                            if($scope.Insmode==='ADD'){
                                     ...
                                     if ($scope.savetype == 1) {
                                        ...
                                        $(':input','#collection_form')
                                        .not(':button, :submit, :reset, :hidden')
                                        .val('')
                                        $scope.collectors = [];
                                        document.querySelector('input[name="txtcoll_code_inter"]').value = $scope.coll_code;
                                        document.querySelector('input[name="txtcoll_year_inter"]').value = $scope.coll_year;
                                        document.querySelector('input[name="txtcoll_number_inter"]').value = $scope.coll_number;
...
*/
    ...
    $scope.demo5 = function(Mode) {
        var savetype = $('input[name="savetype"]:checked').val()
        var pmeters = "tcoll_id=" + encodeURI(document.getElementById("txtcollection_ID_inter").value) +
            /* "&tcoll_full_id=" + encodeURI(document.getElementById("txtcoll_full_id").value) + */
            "&tcoll_start_date=" + encodeURI(document.getElementById("txtcollection_start_date").value) + 
            "&tcoll_end_date=" + encodeURI(document.getElementById("txtcollection_end_date").value) + 
            "&tmethod_method_id=" + encodeURI(document.getElementById("txtcollection_method_ID").value) +
            /*"&tamphur_amphur_id=" + encodeURI(document.getElementById("txtamphur_ID").value) +
            "&ttambon_tambon_id=" + encodeURI(document.getElementById("txtamphur_ID").value) +            
            "&tamphur_direct_id_amphur_direct=" + encodeURI(document.getElementById("txtamphur_direct").value) +
            "&ttambon_direct_id_tambon_pat=" + encodeURI(document.getElementById("txttambon_direct").value) +*/
            "&tcoll_specific_locality=" + encodeURI(document.getElementById("txtspecific_locality").value) + 
            "&tcoll_locality=" + encodeURI(document.getElementById("txtlocality").value) + 
            "&tcoll_thabitat=" + encodeURI(document.getElementById("txthabitat").value) +
            /*"&tcoll_lat_dec=" + encodeURI(document.getElementById("txtlatdec").value) +
            "&tcoll_lat_d=" + encodeURI(document.getElementById("txtlat_d").value) +
            ...
            "&tcoll_northing=" + encodeURI(document.getElementById("txtNorthing").value) + 
            "&tcoll_easting=" + encodeURI(document.getElementById("txtEasting").value) +
            /*"&tdonation_id=" + encodeURI(document.getElementById("txtcoll_code_inter").value) +*/
            "&tcoll_code=" + encodeURI(document.getElementById("txtcoll_code_inter").value) + 
            "&tcoll_year=" + encodeURI(document.getElementById("txtcoll_year_inter").value) + 
            "&tcoll_number=" + encodeURI(document.getElementById("txtcoll_number_inter").value) + 
            "&tMode=" + Mode;
...
/*
 ...
 /*
   $scope.testtest = [
                        {
                            'collectorfirsten': 'Infosys Technologies',
                            'collectorlasten': 125000,
                            'collectoralias': 'Bangalore'
                        },
                        {
                           'collectorfirsten': 'Infosys Technologies',
                            'collectorlasten': 125000,
   ...
   $scope.testtest = [];
    ...
    $scope.addRow = function() {
      ...
      var result = $.grep($scope.collectors, function(e){ return e.collector_id == $scope.myModelCollector.collector_id });
    ...
    $scope.addRowAsyncAsNV = function() {
        ...
        //      
        var data = 'collectorfirsten=' + $scope.myModelCollector.collector_name_en +
                   '&collectorlasten=' + $scope.myModelCollector.collector_name_en + 
                   '&collectoralias=' + $scope.myModelCollector.collector_label_alias;
