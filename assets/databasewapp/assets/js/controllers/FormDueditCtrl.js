 ...
 * AngularJS Directive  
 */
app.controller("FormDueditCtrl", ["$scope", "$http", "$timeout", "$stateParams", "SweetAlert", "test",
    function($scope, $http, $timeout, $stateParams, SweetAlert, test) {
        ...
        var table = $('#DurableListTbl').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": "assets/scripts/server_processing_durableedit.php",
                "type": "POST",
                "data": function(d) {
                    d.sectionid = 1;
                }
            ...
            "columnDefs": [{
                ...
                render: function(data, type, full, meta) {
                    return '<a href="' + "#/app/form/gridform_duedit/" + full[0] + '"' + 'class="btn btn-transparent btn-xs"' + '>' + '<i class="' + 'fa fa-pencil' + '"></i>';
                },
                "targets": 10,
                "width": "6%",
                "orderable": false
        ...
        $scope.DuUpdate = function(action, form) {
            console.log($scope.duModelOwner)
            var a = getIfNotSet($scope.duModelOwner.owner_id, '', true);
            var b = getIfNotSet($scope.duModelRoom.room_id, '', true);
            var c = getIfNotSet($scope.duModelPlace.place_id, '', true);
            var d = dataarr;
            var e = JSON.stringify(d);
            var f = 1;
            var g = getIfNotSet($scope.duModelSection.section_id, '', true);
            var h = getIfNotSet($scope.duModelStatus.status_id, '', true);
            var i = $scope.duModeldurableno;
            var data = $.param({
                tdurable_ids: e,
                troomid: b,
                tplaceid: c,
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    $http({
                        method: 'POST',
                        url: "assets/views/action/du_dbupdate.php",
                        data: data,
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded'
                        },
        ...
        $scope.duModelRoom = '';
        $scope.duModelPlace = '';
        $scope.duModelSection = '';
        $scope.duModelStatus = '';
        $scope.onSelect = function($item, $model, $label) {}
        $scope.OwnerKeyup = function(viewValue) {
            return $http.get('./assets/views/action/ReturnDuDetails.php?sOwner=' + viewValue).then(function(res) {
                return res.data;
            });
        }
        $scope.RoomKeyup = function(viewValue) {
            return $http.get('./assets/views/action/ReturnDuDetails.php?sRoom=' + viewValue).then(function(res) {
                return res.data;
            });
        }
        $scope.PlaceKeyup = function(viewValue) {
            return $http.get('./assets/views/action/ReturnDuDetails.php?sPlace=' + viewValue).then(function(res) {
                return res.data;
            });
        }
        $scope.SectionKeyup = function(viewValue) {
            return $http.get('./assets/views/action/ReturnDuDetails.php?sSection=' + viewValue).then(function(res) {
                return res.data;
            });
        }
        $scope.StatusKeyup = function(viewValue) {
            return $http.get('./assets/views/action/ReturnDuDetails.php?sStatus=' + viewValue).then(function(res) {
                return res.data;
            });
        }
        ...
        $scope.initialise = function() {
            $scope.tabData = [{
                heading: 'One',
                route: 'app.form.test01',
                url: 'user/settings/one/:test',
                controller: 'ExampleCtrl',
                class: 'first-tab'
            }, {
                heading: 'Two',
                route: 'app.form.test02',
                class: 'second-tab'
            }];
        };
        $scope.initialise();
