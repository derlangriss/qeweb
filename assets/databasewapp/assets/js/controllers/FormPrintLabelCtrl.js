 ...
 * AngularJS Directive 
 */
app.controller("FormPrintLabelCtrl", ["$scope", "ngNotify", "$http", "$timeout", "$stateParams", "SweetAlert", "test", "$uibModal", "$log", "printsum", "$state",
    function($scope, ngNotify, $http, $timeout, $stateParams, SweetAlert, test, $uibModal, $log, printsum, $state) {
        ...
        var table = $('#SpecimensListPrintTbl').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "assets/scripts/server_processing_preprintlabel.php",
                "type": "POST"
            },
            ...
            buttons: [{
                ...
                action: function(e, dt, button, config) {
                    $state.go("app.label.form_printedlabel")
                },
                enabled: true
            }],
            "order": [
        ...
        $('#SpecimensListPrintTbl tbody').on('click', '.select-checkbox', function() {
        ...
        function filterColumncollection(i) {
            $('#SpecimensListPrintTbl').DataTable().column(i).search($('#col' + i + '_filter').val(), $('#col' + i + '_regex').prop('checked'), $('#col' + i + '_smart').prop('checked')).draw();
        }
        $('input.column_filter_printcoll').on('keyup', function() {
            filterColumncollection($(this).parents('DIV').attr('data-column'));
        });

        function printqueue(action, id) {
            var data = $.param({
                action: action,
                item_id: id
            });
            $http({
                method: 'POST',
                url: "assets/views/action/deletelabelprintqueue.php",
                data: data,
                headers: {
        ...
        $('#LabelPrintQueue').on("click", '.del', function() {
            ...
            SweetAlert.swal({
                ...
                printqueue("delete", $scope.item_id);
            })
        });
        $scope.config = {
            ...
            $http({
                ...
                angular.forEach($scope.data, function(value, key) {
                    if (value.printed_list == 't') {
                        var newRow = $("<li class='list-group-item-success'>");
