...
if (isset($userid)) {
    ...
    if ($userid !== '') {
        ...
        $strSQL .= "left join boxlockstate on boxlockstate.boxlockstate_id = userlockbox.lockbox_boxstatus ";
        $strSQL .= "left join users_auth on users_auth.uid = userlockbox.lockbox_userid ";
        $strSQL .= "WHERE lockbox_userid <> " . $userid . " ";
        $strSQL .= "AND lockbox_userid > 0";
        ...
        while ($obResult = pg_fetch_array($objQuery)) {
            ...
            for ($i = 0; $i < $intNumField; $i++) {
                ...
                if (pg_field_name($objQuery, $i) == 'lockbox_mreport') {
                	
                    $obResult[$i] = date('M Y', strtotime($obResult[$i]));
                }
