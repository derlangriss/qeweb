...
WHERE collection_code_id = '" . $txtcoll_code . "' AND coll_year ='" . $txtcoll_year . "' AND coll_number ='" . $txtcoll_number . "'";
        ...
        if ($intRows > 0) {
            ...
            $sql_check_specimens = "SELECT specimens_number+1 AS specno,coll_id,coll_code, coll_year, coll_number FROM specimens
left join collection on collection.coll_id = specimens.collection_coll_id
LEFT JOIN collection_code ON collection.collection_code_collection_code_id = collection_code.collection_code_id
WHERE collection_code_id = '" . $txtcoll_code . "' AND coll_year ='" . $txtcoll_year . "' AND coll_number ='" . $txtcoll_number . "'
ORDER BY specimens_number desc
LIMIT 1";
            $res_check_specimens     = pg_query($sql_check_specimens);
            $intRows_check_specimens = pg_num_rows($res_check_specimens);
            if ($intRows_check_specimens > 0) {

                $row_check_specimens = pg_fetch_array($res_check_specimens);
                extract($row_check_specimens);
                $txtcoll_number = sprintf('%04d', $coll_number);
                $txtspec_number = sprintf('%04d', $specno);

                $arr = array('coll_id' => $coll_id,'coll_code' => $coll_code, 'coll_year' => $coll_year, 'coll_number' => $txtcoll_number, 'spec_number' => $txtspec_number, 'gen_mode' => 3);
...
WHERE collection_code_id = '" . $txtcoll_code . "' AND coll_year ='" . $txtcoll_year . "'";
        ...
        if ($intRows > 0) {
            ...
            $sql = "SELECT coll_code, coll_year, coll_number FROM specimens
LEFT JOIN collection on collection.coll_id = specimens.collection_coll_id
left join collection_code ON collection.collection_code_collection_code_id = collection_code.collection_code_id
WHERE collection_code_id = '" . $txtcoll_code . "' AND coll_year ='" . $txtcoll_year . "'";
            ...
            if ($intRows > 0) {
                ...
                $sql2 = "SELECT specimens_number+1 as specno,coll_number,coll_code,coll_year,collection_code_id,coll_id
                     FROM specimens
...
ORDER BY coll_number desc , specno desc
limit 1";
                ...
                $arr = array('coll_id' => $coll_id,'coll_code' => $coll_code, 'coll_year' => $coll_year, 'coll_number' => $txtcoll_number, 'spec_number' => $txtspec_number, 'gen_mode' => 2);
                ...
                if ($txtcoll_year <= $current_year) {
                    ...
                    $sql = "SELECT coll_id,coll_code, coll_year, coll_number FROM collection
left join collection_code ON collection.collection_code_collection_code_id = collection_code.collection_code_id
WHERE collection_code_id = '" . $txtcoll_code . "' AND coll_year ='" . $txtcoll_year . "' ORDER BY coll_number DESC limit 1";
                    $res = pg_query($sql);
                    $row = pg_fetch_array($res);
                    extract($row);
                    ...
                    $newcount       = 1;
                    $txtcoll_number = sprintf('%04d', $coll_number);
                    $txtspec_number = sprintf('%04d', $newcount);
...
$sql2 = "SELECT DISTINCT coll_number,coll_code,coll_year FROM collection
