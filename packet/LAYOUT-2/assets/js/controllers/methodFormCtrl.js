 ...
 */
app.controller('methodFormCtrl', ["$http", "$scope", "$location", function($http, $scope, $location, $state) {
    var selected = [];
    var table = $('#methodTbl').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": {
            "url": "assets/scripts/server_processing_method.php",
            "type": "POST"
        },
        "scrollX": true,
        "columns": [{
    ...
    $scope.Methodins = function(Mode) {
        var a = $("#txtmethod").val();
        var b = $("#txtmethod_note").val();
        ...
        */
        $http({
            method: 'POST',
            url: 'assets/views/action/dbinsertMethod.php',
            data: data,
            headers: {
