 ...
 * AngularJS Directive 
 */
app.controller("FormPrintedLabelCtrl", ["$scope", "ngNotify", "$http", "$timeout", "$stateParams", "SweetAlert", "test", "$uibModal", "$log", "printsum", "$state",
    function($scope, ngNotify, $http, $timeout, $stateParams, SweetAlert, test, $uibModal, $log, printsum, $state) {
        ...
        var tableprinted = $('#SpecimensListPrintTbl').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
            ...
            "buttons": [{
                ...
                action: function(row) {
                    ...
                    if (dataarr.length > 0) {
                        tableprinted.rows().deselect();
                        $("#checkitem").addClass('fa-square-o');
                        $("#checkitem").removeClass('fa-check-square-o');
                        dataarr = [];
                    } else {
                        tableprinted.rows().select();
                        $(row).addClass('selected');
                        var data = tableprinted.rows('.selected').select().data();
