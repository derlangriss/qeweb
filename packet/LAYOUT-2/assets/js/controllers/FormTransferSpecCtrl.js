...
app.controller("FormTransferSpecCtrl", ["$scope", "$http", "$timeout", "$stateParams", "$state", "SweetAlert", "info", "ngNotify", "ServicePDF", "$aside",
    function($scope, $http, $timeout, $stateParams, $state, SweetAlert, info, ngNotify, ServicePDF, $aside) {
                        ...
                        if (isConfirm) {
                            $http({
                                ...
                                SweetAlert.swal({
                                    ...
                                    window.onkeydown = previousWindowKeyDown;
                                    dataarrmuseumrest = [];
                                    tableSpecMuseumRestList.draw(false);
                                    tableSpecReportList.draw(false);
                                    $scope.countSpecimens(monthshow, yearshow, null, null);
                                    $("#checkitemres").removeClass('fa-check-square-o');
                                    $("#checkitemres").addClass('fa-square-o');
        ...
        var tableSpecReportList = $('#SpecimensReportListTbl').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": "assets/scripts/server_processing_resroom.php",
                "type": "POST",
                "data": function(d) {
                    d.treport_month = getIfNotSet($stateParams.monthid, '', true);
                    d.treport_year = getIfNotSet($stateParams.yearid, '', true);
                    d.tres_room = getIfNotSet($scope.ctype_resroom, 0, true);
                    d.tmuseumstatus = getIfNotSet($scope.museumstatus, 0, true);
                }
            },
            "createdRow": function(row, data, index) {
                if (data[1] == 'Unknown') {
...
*/
        ...
        var tableSpecMuseumRestList = $('#SpecimensMuseumRestTbl').DataTable({
            ...
            "rowCallback": function(row, data) {
                var rowid = data.DT_RowId;
                var rowidres = rowid.substring(4);
                if ($.inArray(rowidres, dataarrmuseumrest) !== -1) {
                    $(row).addClass('selected');
                }
            },
            "drawCallback": function(settings) {
            ...
            "buttons": [{
                ...
                action: function(row) {
                    $(row).removeClass('selected');
                    if (dataarrmuseumrest.length > 0) {
                        tableSpecMuseumRestList.rows().deselect();
                        $("#checkitemres").addClass('fa-square-o');
                        $("#checkitemres").removeClass('fa-check-square-o');
                        dataarrmuseumrest = [];
                    } else {
                        tableSpecMuseumRestList.rows().select();
                        $(row).addClass('selected');
            ...
            for (var x = 0; x < columns.length; x++) {
                tableSpecMuseumList.column(columns[x], {
                    ...
                    if (order === data[1] && family === data[2]) {
                        ...
                        if ($(tr).hasClass('selected')) {
                            $(tr).removeClass('selected');
                            tableSpecReportList.rows(dataarrmuseum).deselect();
                        }
                    }
                });
            }
        ...
        $('#SpecimensMuseumRestTbl tbody').on('click', '.checkallres', function() {
            ...
            var orderarr = [];
            for (var x = 0; x < columns.length; x++) {
                tableSpecMuseumRestList.column(columns[x], {
                    page: 'current'
                }).data().each(function(group, i) {
                    var data = tableSpecMuseumRestList.row(i).data();
                    if (order === data[1] && family === data[2]) {
                        var id = data.DT_RowId;
                        var idres = "#" + id;
                        var idpure = id.substring(4);
            ...
            for (var x = 0; x < columns.length; x++) {
                tableSpecMuseumRestList.column(columns[x], {
                    ...
                    if (order === data[1] && family === data[2]) {
                        ...
                        if ($(tr).hasClass('selected')) {
                            $(tr).removeClass('selected');
                            tableSpecReportList.rows(dataarrmuseumrest).deselect();
                        }
                    }
                });
            }
        ...
        $('#SpecimensMuseumRestTbl tbody').on('click', '.select-checkbox', function() {
            var tr = $(this).closest('tr');
            var data = tableSpecMuseumRestList.row($(this).closest('tr')).data();
            var id = data.DT_RowId;
            var idres = id.substring(4);
            ...
            if ($(tr).hasClass('selected')) {
                $(tr).removeClass('selected');
                tableSpecMuseumRestList.rows($(this).closest('tr')).deselect();
            } else {
                $(tr).addClass('selected');
            }
        });
