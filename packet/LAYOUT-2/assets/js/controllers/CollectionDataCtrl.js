...
app.controller("CollectionDataCtrl", ["$scope", "ngNotify", "$http", "$timeout", "$stateParams", "SweetAlert", "info", "$state", 'FileUploader', 'ngNotify', function($scope, ngNotify, $http, $timeout, $stateParams, SweetAlert, info, $state, FileUploader, $window) {
    ...
    $scope.gencoll_code = "1";
    $scope.tabiden = 1;
    $scope.setTabIden = function(idenTab) {
        if (idenTab == 1) {
            $scope.acceptable = 1;
        } else {
            $scope.acceptable = 0;
    ...
    $scope.GETNERATEAUTOCOLLNO = function() {
        ...
        $http({
            method: 'POST',
            url: "assets/views/action/return_collnumber_collection.php",
            data: data,
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
            ...
            if (result !== '') {
                $scope.data.coll_number = result[0].coll_number;
                $scope.gencoll_number = result[0].coll_number;
            }
        });
    }
    $scope.GETNERATEAUTOCOLLNOex = function() {
        var a = getIfNotSet($scope.gencoll_code, '', true);
        var b = getIfNotSet($scope.gencoll_year, '', true);
        var c = getIfNotSet($scope.gencoll_number, '', true);
        var data = $.param({
    ...
    $scope.getCollectionname = function(id) {
        for (var i = 0; i < $scope.GETcollectioncode.length; i++) {
            if ($scope.GETcollectioncode[i].collection_code_id == id) {
                return $scope.GETcollectioncode[i].collection_code;
            }
        }
        return "";
    }
    $scope.CollectionData = function(form) {
        var collectioncodename =   $scope.getCollectionname($scope.gencoll_code)
        ...
        var data = $.param({
            ...
            tcoll_lat_dec: $scope.data.collectionlatdec,
            ...
            tcoll_long_dec: $scope.data.collectionlongdec,
            ...
            tcoll_code: collectioncodename,
            tcoll_year: $scope.gencoll_year,
                        ...
                        SweetAlert.swal({
                            ...
                            if ($scope.Insmode === 'ADD') {
                                if ($scope.savetype == 1) {
                                    ...
                                    $scope.txtcollection_end_date = '';
                                    $scope.collectors = [];
                                    $scope.gencoll_code = response[0].coll_code;
                                    $scope.gencoll_year = response[0].coll_year;
