 ...
 */
app.controller('reporttab', ["$scope", "$http", "$timeout", "$state", "$stateParams", "ServicePDF","ServiceExcel", "$element",
    function($scope, $http, $timeout, $state, $stateParams, ServicePDF,ServiceExcel, $element) {
        function monthstr() {
            var month = new Array();
            month[0] = "January";
            month[1] = "February";
        ...
        $scope.changeDateMonthEX = function(month, year) {
            if (month && year !== '0') {
                $state.go('app.specimens.specimensoverview', {
                    "yearid": year
                })
            }
        };
        ...
        $scope.ExportExcel = function(year) {
        
            var datenow = new Date();
            var name = "ReportExcel";
            var getyear = datenow.getFullYear();
            var getdate = datenow.getDate();
            var getmonth = monthstr();
            var gethours = datenow.getHours();
            ...
            panel.addClass(csspinnerClass + " " + spinner)
            $timeout(function() {
                ServiceExcel.downloadExcel(year).then(function(result) {
                    var file = new Blob([result.data], {
        ...
        $scope.formatDateReportM = function(datereport) {
            var a = datereport.getMonth();
            var month = new Array();
            month[0] = "January";
            month[1] = "February";
        ...
        var yearfiscal = getIfNotSet($stateParams.yearid, 0, true)
        $http({
            method: 'GET',
            url: 'assets/views/action/getSpecFiscalyear.php',
            params: { year_id: yearfiscal }
        ...
        $scope.countYearSpecimens = function() {
            $http({
                ...
                var avgfamilymaxcount = ($scope.countmaxfamily / $scope.countallyear) * 100
                $scope.avgfamilymaxcountdisplay = avgfamilymaxcount.toFixed(2)
                var avgbarknob = ($scope.countallyearfamily / 80) * 100
                $scope.avgbarknobdisplay = avgbarknob.toFixed(2)
                ...
                var monthdes = monthdis - 1
                var datereport = new Date(yearshow, monthdes);
                $scope.countexpecyear = $scope.formatDateReportM(datereport);
        ...
        */
        $scope.countFiscalYear = function() {
            var yearhtml = getIfNotSet($stateParams.yearid, 0, true);
            $http({
                method: 'GET',
                url: 'assets/views/action/countFiscalYear.php',
                params: {
                    year_id: yearhtml
                }
            }).success(function(response) {
                ...
                $scope.countmaxmonth = response[0].countex
                $scope.countmaxfamilyname = response[0].exfamily
                $scope.beginbudget = response[0].beginbudget
                $scope.endbudget = response[0].endbudget
                $scope.ENfiscalyear = response[0].ENfiscalyear
                $scope.THfiscalyear = response[0].THfiscalyear

                $scope.thbeginbudget = response[0].thbeginbudget
                $scope.thendbudget = response[0].thendbudget
        ...
        $scope.LabelSearch = function(m, y) {
            ...
            $.fn.dataTable.ext.buttons.exexcel = {
                className: 'buttons-alert',
                action: function(e, dt, node, config) {
                    $scope.ExportExcel($stateParams.yearid)
                }
            };

        $('#familyfiscalTbl').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "assets/scripts/server_processing_fiscalyear.php",
                "type": "POST",
                "data": function(d) {
            ...
            "pagingType": "input",
            "dom": "<'row'<'col-sm-6'<'pull-left'fB>><'col-sm-6'p>>" + "<'row'<'col-sm-12 margin-bottom-10'tr>>",
            "buttons": [{
