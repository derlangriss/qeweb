 ...
 * AngularJS Directive  
 */
app.controller("SpecManageIdentifyCtrl", ["$scope", "$uibModal", "$log", "$http", "$interval", "$timeout", "$stateParams", "$state", "SweetAlert", "info", "ngNotify", "ServicePDF", "$aside", "toaster",
    function($scope, $uibModal, $log, $http, $interval, $timeout, $stateParams, $state, SweetAlert, info, ngNotify, ServicePDF, $aside, toaster) {
        ...
        $scope.GetSelectBoxDetails = function(a, b) {
            var user_id = getIfNotSet($scope.uid, 0, true)
            $http({
                method: 'GET',
                url: 'assets/views/action/GetSelectBoxDetails.php',
                params: {
        ...
        var specboxid = getIfNotSet($scope.BoxShowDetailsBoxid, 0, true)
        $scope.GetSelectBoxDetails(specboxid, 1)
        console.log($scope.BoxShowDetailsBoxid)
        ...
        $scope.backstep1 = function(id) {
            if (id !== undefined) {
                ...
                $scope.BoxShowDetailsBoxid = ''
                $scope.GETAllcollboxno = 'All'
                $scope.idenboxlist();
                $scope.countSpecimens(monthshow, yearshow, null, null);
                $scope.GETAllundefinedbox = ''
                $scope.GETAllcountspecinbox = ''
                $scope.GetSelectBoxDetails(0, 1)
            }
        }
        ...
        function ModalSelectSpecies(a, b, c, d, e, f, g, h, i) {
            ...
            $http({
                ...
                tableSpecimensBox.draw();
                tableSpecIdList.draw();
                $scope.countSpecimens(d, e, null, null);
                $scope.GetUserCurrentBox();
            });
        }
        $("#SpecimensIdList tbody").on("click", "a.single-print", function(event) {
            ...
            var oldordername = dataspecimens[6];
            var oldfamilyname = dataspecimens[7];
            var oldgenusname = dataspecimens[8];
            var oldspeciesname = dataspecimens[9];
            var neworderid = getIfNotSet($scope.specModelOrder.torder_id, 0, true);
            var newfamilyid = getIfNotSet($scope.specModelFamily.family_id, 0, true);
            var newgenusid = getIfNotSet($scope.specModelGenus.genus_id, 0, true);
            var newspeciesid = getIfNotSet($scope.specModelSpecies.species_id, 0, true);
            var newordername = getIfNotSet($scope.specModelOrder.torder_name, 'Unknown', true);
            var newfamilyname = getIfNotSet($scope.specModelFamily.family_name, 'Unknown', true);
            var newgenusname = getIfNotSet($scope.specModelGenus.genus_name, 'Unknown', true);
            var newspeciesname = getIfNotSet($scope.specModelSpecies.species_name, 'Unknown', true);
            var containerid = getIfNotSet($scope.BoxShowDetailsBoxid, '0', true);
            var containertype = 1;
            var report_year = $stateParams.yearid
            var report_month = $stateParams.monthid
            $scope.open = function(size) {
                var modalInstance = $uibModal.open({
                    ...
                    controller: function($scope, $uibModalInstance) {
                        ...
                        $scope.modalContent = "";
                        $scope.oldordername = oldordername;
                        $scope.oldfamilyname = oldfamilyname;
                        $scope.oldgenusname = oldgenusname;
                        $scope.oldspeciesname = oldspeciesname;
                        $scope.oldspeciesid = oldspeciesid;
                        $scope.newordername = newordername;
                        $scope.newfamilyname = newfamilyname;
                        $scope.newgenusname = newgenusname;
                        $scope.newspeciesname = newspeciesname;
                        $scope.newspeciesid = newspeciesid;
                        $scope.specimensid = specimensid;
                        $scope.containerid = containerid;
                        $scope.containertype = containertype;
                        $scope.year = report_year;
                        $scope.month = report_month;
                        $scope.SelectData = function(a) {
                            if (a === 'oldspec') {
                                ModalSelectSpecies($scope.specimensid, $scope.containerid, $scope.containertype, report_month, report_year, oldorderid, oldfamilyid, oldgenusid, oldspeciesid);
                                $uibModalInstance.close();
                            }
                            if (a === 'newspec') {
                                ModalSelectSpecies($scope.specimensid, $scope.containerid, $scope.containertype, report_month, report_year, neworderid, newfamilyid, newgenusid, newspeciesid);
                                $uibModalInstance.close();
                            }
                        }
            ...
            var data = $.param({
                ...
                tcontainer_id: containerid,
                tcontainer_type: containertype,
                treport_month: report_month,
                treport_year: report_year,
                torderid: neworderid,
                tfamilyid: newfamilyid,
                tgenusid: newgenusid,
                tspeciesid: newspeciesid,
            });
            if (typeof user_id !== 'undefined' && container_id !== '0') {
