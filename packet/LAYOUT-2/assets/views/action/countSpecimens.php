...
if (isset($_GET) && count($_GET)) {
    ...
    $strSQL01ex = "SELECT count(specimens_id) AS allspecimensresroom FROM specimens ";
    $strSQL01ex .= "where specimens_trash = 1 AND container_type = 1 AND EXTRACT(MONTH FROM sreport_date) = " . $month . " AND EXTRACT(YEAR FROM sreport_date) = " . $year;
    $objQuery01ex = pg_query($strSQL01ex);

    $row01ex = pg_fetch_array($objQuery01ex);
    extract($row01ex);

    $strSQL02ex = "SELECT count(specimens_id) AS resspecimensonresroom FROM specimens ";
    $strSQL02ex .= "where specimens_trash = 1 AND container_type = 2 AND EXTRACT(MONTH FROM sreport_date) = " . $month . " AND EXTRACT(YEAR FROM sreport_date) = " . $year;
    $objQuery02ex = pg_query($strSQL02ex);

    $row02ex = pg_fetch_array($objQuery02ex);
    extract($row02ex);
    ...
    $strSQL03 .= "where specimens_trash = 1 AND EXTRACT(MONTH FROM sreport_date) = " . $month . " AND EXTRACT(YEAR FROM sreport_date) = " . $year . " ";
    ...
    $strSQL07 = "SELECT count(DISTINCT lockbox_boxid) AS countbox FROM userlockbox ";
    $strSQL07 .= "where EXTRACT(MONTH FROM lockbox_mreport) = " . $month . " AND EXTRACT(YEAR FROM lockbox_mreport) = " . $year . " ";
    $strSQL07 .= "AND active_state = 'TRUE' ";
    ...
    $arr = array('success' => '1', 'allbox' => $countbox, 'allspecimensresroom' => $allspecimensresroom, 'resspeconresroom' => $resspecimensonresroom, 'resspecimens' => $resspecimens, 'allspecimens' => $allspecimens, 'undefinedspecimens' => $undefinedspecimens, 'countregist' => $countregist, 'countmanaged' => $countmanaged, 'countfamily' => $countfamily, 'specinbox' => $specinbox, 'undefinedspecinbox' => $undefinedspecinbox, 'boxstatus' => $boxstatus);
