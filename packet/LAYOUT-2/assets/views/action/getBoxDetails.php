...
if (isset($_GET['userid'])) {
    ...
    $strSQL = "SELECT collbox_id,collboxno,firstname,boxstatus,lockbox_mreport,state_color,boxlocate,lockbox_lockboxstate,avartar,lockbox_userid FROM userlockbox ";
    ...
    while ($obResult = pg_fetch_array($objQuery)) {
        ...
        $strSQLCountSpecimensInbox .= "SELECT (SELECT counteachbox from countallfullqty) ,(SELECT countspec from countspecinbox) , (SELECT countundefinedspec from countundefinedspecinbox)";
        ...
        if ($lockbox_lockboxstate == 1) {
            $arrCol['boxstate'] = "fa-lock";
        } else {
            $arrCol['boxstate'] = "fa-unlock-alt";
        }
        $arrCol['user_name'] = strtoupper(substr($firstname, 0, 3));
        ...
        while ($obResult01 = pg_fetch_array($objQuery02)) {
            ...
            if ($userid === $lockbox_userid) {
                $arrCol['compareUser'] = 'fa-hand-o-right';
            } else {
                if ($lockbox_userid === '0') {
                    $arrCol['compareUser'] = '';
                } else {
                    $arrCol['compareUser'] = 'fa-retweet';
                }
